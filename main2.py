            ...
            try:
                ...
                if auth_header and auth_header.startswith('Bearer '):
                    ...
                    if 'auth' in get_params:
                        full_auth = get_params['auth'][0]  # e.g. "user1:0XAXAXAQX5A1F/v1/"
                        if '/' in full_auth:
                            token, path_override = full_auth.split('/', 1)
                            path_override = '/' + path_override  # "/v1/"
                        else:
                            token = full_auth
                ...
                if authorized_users.get(user) == key:
                    self.user = user
                    # Override self.path if path_override is present
                    if path_override:
                        # Rebuild path with overridden path and original query without auth param
                        url = urlparse(self.path)
                        # Remove auth param from query
                        query_params = parse_qs(url.query)
                        query_params.pop('auth', None)
                        # Rebuild query string without auth
